...
| M9 | **Distribution and production readiness** — release builds + backend-hosted downloads, pairing/token helper auth, tray + autostart comfort, backend hardening; non-developers join without a dev setup | ◐ (P1 download pipeline + P2 pairing/token auth + P3 tray/autostart done; P4 hardening next) |
