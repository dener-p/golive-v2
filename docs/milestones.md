...
| M9 | **Distribution and production readiness** — release builds + backend-hosted downloads, pairing/token helper auth, tray + autostart comfort, backend hardening; non-developers join without a dev setup | ◐ (P1 download pipeline + P2 pairing/token auth + P3 tray/autostart + P4 hardening done; release exe validated end-to-end through a public tunnel; notebook deployment (LAN + phone Wi-Fi) + friend's-PC re-test pending) |
...
- [x] Tray icon + "start with Windows" toggle (tray-icon crate on its own thread: open host page / start-stop / NAT self-test / start-with-Windows / quit; favicon-derived 32×32 icon in `helper/assets/tray.png`; HKCU Run via `reg.exe`; `GOLIVE_NO_TRAY=1` to disable headless; 24 helper tests pass)
- [x] Backend hardening: `SESSION_SECRET` fail-fast (non-dev-auth boot guard), `GET /healthz`, per-IP rate limits (`rateLimit.ts`: auth/login, auth/dev, room:create, pair:mint, pair:exchange — 429 + `Retry-After`, `RATE_LIMITS=off` escape hatch), room ids 6→10 chars (~50-bit, unguessable links); 51 server tests pass, typecheck clean except pre-existing `helperSignaling.test.ts` recursion-only failures
